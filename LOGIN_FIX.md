...
- API服务已在后台启动：http://localhost:43201
...
```bash
curl -X POST http://localhost:43201/api/auth/login \
...
- 确认API服务是否正常运行（访问 http://localhost:43201/api/docs）
- 确认数据库连接是否正常
