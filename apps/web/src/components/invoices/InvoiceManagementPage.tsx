...
import { getTaxRateDisplay, isContractMatchedDirection, resolveAttachmentUrl, resolveDirectionByContractType } from './utils';
...
export default function InvoiceManagementPage({ fixedDirection }: InvoiceManagementPageProps) {
  ...
  const extractImportErrors = useCallback((error: unknown): InvoiceImportError[] => {
  ...
  }, [contracts, fetchInvoices, fixedDirection, form, uploadedFile?.filename, uploadedFile?.url]);
