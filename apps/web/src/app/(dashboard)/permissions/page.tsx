...
export default function PermissionsPage() {
  ...
  const [rolePermissions, setRolePermissions] = useState<Record<string, { menus: string[]; functions: string[] }>>({});
  ...
  const getCurrentMatrix = () => {
    return Object.keys(rolePermissions).length > 0 ? rolePermissions : ROLE_MENU_MATRIX_DEFAULT;
  };
  ...
  return (
    <div>
      ...
      <Modal
        ...
        width={700}
        footer={[
          <Button key="reset" onClick={() => editingRole && handleReset(editingRole)}>
            重置为默认
          </Button>,
          <Button key="cancel" onClick={() => setEditModalVisible(false)}>
            取消
          </Button>,
          <Button key="save" type="primary" loading={saving} onClick={handleSave} icon={<SaveOutlined />}>
            保存
          </Button>,
        ]}
      >
                ...
                <Checkbox
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      setEditingMenus(editingMenus.filter((k) => k !== menu.key));
                    }
                  }}
                >
                  {menu.name}
                ...
                <Checkbox
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      setEditingFunctions(editingFunctions.filter((k) => k !== func.key));
                    }
                  }}
                >
                  {func.name}
