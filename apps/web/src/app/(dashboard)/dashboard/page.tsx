  ...
  return (
    <Card className="card-hover tech-stat-card">
      <div className="flex items-start justify-between">
        <div>
          <Text className="text-sm !text-white/85">{title}</Text>
          <div className="mt-2">
            <Statistic
              value={value}
              prefix={prefix}
...
export default function DashboardPage() {
  ...
  const [contractDashboard, setContractDashboard] =
    useState<ContractDashboard | null>(null);
  const [expenseAnalysis, setExpenseAnalysis] =
    useState<ExpenseAnalysis | null>(null);
  const [paymentStats, setPaymentStats] =
    useState<PaymentRequestStatistics | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchReceivables = async () => {
      try {
        const res = await api.get<ReceivablesOverview>("/reports/receivables");
        setReceivables(res);
      } catch (error: unknown) {
        console.error("加载应收数据失败:", getErrorMessage(error));
      } finally {
    ...
    const fetchContracts = async () => {
      try {
        const res = await api.get<ContractDashboard>(
          "/reports/contracts/dashboard",
        );
        setContractDashboard(res);
      } catch (error: unknown) {
        console.error("加载合同数据失败:", getErrorMessage(error));
      } finally {
    ...
    const fetchExpenses = async () => {
      try {
        const res = await api.get<ExpenseAnalysis>("/reports/expenses");
        setExpenseAnalysis(res);
      } catch (error: unknown) {
        console.error("加载报销数据失败:", getErrorMessage(error));
      } finally {
    ...
    const fetchPaymentRequests = async () => {
      try {
        const res = await api.get<PaymentRequestStatistics>(
          "/payment-requests/statistics",
        );
        setPaymentStats(res);
      } catch (error: unknown) {
        console.error("加载付款统计失败:", getErrorMessage(error));
      } finally {
    ...
    const fetchEntityCounts = async () => {
      try {
        ...
        console.error("加载客户/供应商统计失败:", getErrorMessage(error));
      } finally {
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <Text className="!text-[#d8ecff]">
                今天是 {currentDate}，可在左侧菜单提交和追踪报销进度。
              </Text>
            </div>
          </div>
        </div>
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card loading={loadingMap.contracts} className="tech-stat-card">
              <Statistic
                title={<span className="text-white/85">执行中销售合同</span>}
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card loading={loadingMap.contracts} className="tech-stat-card">
              <Statistic
                title={<span className="text-white/85">本月新签销售合同</span>}
      ...
      <Card title="💵 收款模块" className="mt-4">
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Card loading={loadingMap.contracts} className="tech-stat-card">
              <Statistic
                title={<span className="text-white/85">本月销售合同回款</span>}
                value={formatAmount(
                  contractDashboard?.monthlyPaymentAmount || 0,
                )}
                prefix="¥"
                valueStyle={{ color: "#ffffff" }}
              />
            ...
            <Card title="⏰ 即将到期销售合同回款（7天内）">
              {loadingMap.contracts ? (
                ...
                <Table
                  ...
                  locale={{ emptyText: "暂无即将到期的销售合同回款" }}
                />
              )}
            </Card>
          </Col>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <Text strong className="text-green-500">
                      ¥
                      {formatAmount(
                        receivables?.agingDistribution?.normal || 0,
                      )}
                    </Text>
                  </div>
                  <div className="flex justify-between items-center">
              ...
              {loadingMap.expenses ? (
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <Text>本月报销总额</Text>
                    <Text strong>
                      ¥{formatAmount(expenseAnalysis?.monthlyTotal || 0)}
                    </Text>
                  </div>
                  <div className="flex justify-between items-center">
                    <Text>待审批</Text>
              ...
              {loadingMap.paymentRequests ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    <Text>待审批</Text>
                    <Text strong className="text-orange-500">
                      {paymentStats?.pendingCount || 0} 笔
                    </Text>
                  </div>
                  <div className="flex justify-between items-center">
                    <Text>已通过待付</Text>
                    <Text strong className="text-blue-500">
                      {paymentStats?.approvedCount || 0} 笔
                    </Text>
                  </div>
                  <div className="flex justify-between items-center">
                    <Text>已付款</Text>
                    <Text strong className="text-green-500">
                      {paymentStats?.paidCount || 0} 笔
                    </Text>
                  </div>
                  <div className="flex justify-between items-center">
                    <Text>已驳回</Text>
                    <Text strong className="text-red-500">
                      {paymentStats?.rejectedCount || 0} 笔
                    </Text>
                  </div>
                </div>
              )}
