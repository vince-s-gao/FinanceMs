...
describe("UploadService", () => {
  ...
  beforeEach(() => {
    uploadRoot = path.join(
      os.tmpdir(),
      `upload-test-${Date.now()}-${Math.random()}`,
    );
    const configService = {
  ...
  it("should save valid file and return metadata", async () => {
    ...
    expect(result.url).toBe(
      "/uploads/contracts/11111111-1111-4111-8111-111111111111.pdf",
    );
    expect(
      fs.existsSync(
        path.join(
  ...
  it("should use default temp category when saveFile category is omitted", async () => {
    ...
    expect(result.url).toBe(
      "/uploads/temp/11111111-1111-4111-8111-111111111111.pdf",
    );
    expect(
      fs.existsSync(
        path.join(
  ...
  it("should reject saveFile when resolved path escapes upload dir", async () => {
    const sanitizeSpy = jest
      .spyOn(service as any, "sanitizePath")
      .mockReturnValue("../evil");
  ...
  it("should reject deleteFile when resolved path escapes upload dir", async () => {
    ...
    await expect(
      service.deleteFile("/uploads/temp/escape.pdf"),
    ).rejects.toThrow(BadRequestException);
  ...
  it("should reject getFilePath when resolved path escapes upload dir", () => {
    ...
    expect(() => service.getFilePath("/uploads/contracts/a.pdf")).toThrow(
      BadRequestException,
    );
  ...
  it("should return false for unknown mime in private validators", () => {
    expect(
      (service as any).validateFileMagicNumber(
        Buffer.from([0x01, 0x02]),
        "application/x-unknown",
      ),
    ).toBe(false);
    expect(
      (service as any).validateFileExtension(
        "a.unknown",
        "application/x-unknown",
      ),
    ).toBe(false);
