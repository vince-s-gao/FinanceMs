...
export class ExpensesController {
  ...
  async create(@Body() createExpenseDto: CreateExpenseDto, @CurrentUser() user: any) {
    return this.expensesService.create(createExpenseDto, user.id, user.department || '未分配');
  }
  ...
  async update(
    ...
    return this.expensesService.update(id, updateExpenseDto, user.id, user.role);
  }
  ...
  async approve(@Param('id') id: string, @Body() approveDto: ApproveExpenseDto) {
