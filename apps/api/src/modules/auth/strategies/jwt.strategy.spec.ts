...
describe("JwtStrategy", () => {
  const buildConfigService = (
    env = "test",
    secret = "test-jwt-secret-key-with-enough-length-123",
  ) => ({
  ...
  it("should reject startup in production when jwt secret is weak", () => {
    ...
    expect(
      () => new JwtStrategy(configService as any, authService as any),
    ).toThrow("JWT_SECRET 配置无效或过弱，生产环境禁止启动");
  });
  ...
  it("should reject non-access token type", async () => {
    ...
    await expect(
      strategy.validate({ sub: "u1", type: "refresh" }),
