...
describe('FeishuService', () => {
  ...
  beforeEach(() => {
    configService = {
      get: jest.fn((key: string, fallback?: string) => {
        ...
        if (key === 'FEISHU_REDIRECT_URI') return 'https://example.com/callback';
  ...
  it('should create and exchange login ticket once', () => {
    ...
    expect(() => service.exchangeLoginTicket(ticket)).toThrow(UnauthorizedException);
  });
  ...
  it('should cleanup expired tickets when creating a new ticket', () => {
    ...
    expect(() => service.exchangeLoginTicket(expiredTicket)).toThrow(UnauthorizedException);
    expect(service.exchangeLoginTicket(validTicket).accessToken).toBe('new-token');
    jest.useRealTimers();
  });
  ...
  it('should throw when getting app access token fails', async () => {
    ...
    await expect((service as any).getAppAccessToken()).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('should reject feishu login when user is inactive', async () => {
    ...
    await expect(service.loginWithFeishu('code-3')).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('should reject bind when feishu account already bound', async () => {
    ...
    await expect(service.bindFeishuAccount('current-user', 'code-x')).rejects.toThrow(
  ...
  it('should bind feishu account successfully', async () => {
    ...
    expect(result).toEqual({ message: '飞书账号绑定成功', feishuUserId: 'user-y' });
  });

  it('should reject unbind when user not exists or has no password', async () => {
    prisma.user.findUnique.mockResolvedValueOnce(null);
    await expect(service.unbindFeishuAccount('missing')).rejects.toThrow(UnauthorizedException);

    prisma.user.findUnique.mockResolvedValueOnce({ id: 'u1', password: null });
    await expect(service.unbindFeishuAccount('u1')).rejects.toThrow(UnauthorizedException);
  });

  it('should unbind feishu account successfully', async () => {
    prisma.user.findUnique.mockResolvedValueOnce({ id: 'u1', password: 'hashed' });
