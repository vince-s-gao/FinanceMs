...
export class AuthController {
  ...
  private parseDurationToSeconds(value: string, fallbackSeconds: number): number {
  ...
  async feishuCallback(
    ...
    const stateInCookie = this.getCookieValue(req.headers.cookie, 'feishu_oauth_state');
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const refreshToken = this.getCookieValue(req.headers.cookie, 'refreshToken');
    if (!refreshToken) {
  ...
  async bindFeishu(@CurrentUser() user: any, @Body() feishuLoginDto: FeishuLoginDto) {
