...
describe("ProjectsService", () => {
  ...
  it("should evaluate isProjectCodeConflict helper branches", () => {
    ...
    expect((service as any).isProjectCodeConflict(nonConflictError)).toBe(
      false,
    );
    ...
    expect((service as any).isProjectCodeConflict(wrongTargetError)).toBe(
      false,
    );
    ...
    expect((service as any).isProjectCodeConflict(missingMetaError)).toBe(
      false,
    );
    ...
    expect((service as any).isProjectCodeConflict(codeConflictError)).toBe(
      true,
    );
  });
  ...
  it("should throw when update target project does not exist", async () => {
    jest
      ...
      .mockRejectedValueOnce(new NotFoundException("项目不存在"));
