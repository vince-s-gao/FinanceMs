...
describe('PaymentsService', () => {
    ...
    prisma.paymentPlan.findMany.mockResolvedValueOnce([
      {
        ...
        paymentRecords: [{ amount: new Decimal(120) }, { amount: new Decimal(80) }],
      },
    ]);
  ...
  it('should throw when createPlans contract does not exist', async () => {
    ...
    await expect(service.createPlans('missing', [])).rejects.toThrow(NotFoundException);
  });
  ...
  it('should reject createPlans when total exceeds contract amount', async () => {
    ...
    await expect(
      service.createPlans('c1', [
        { period: 1, planAmount: new Decimal(200), planDate: '2026-01-01' } as any,
        { period: 2, planAmount: new Decimal(200), planDate: '2026-02-01' } as any,
      ]),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('should replace plans in createPlans when validations pass', async () => {
    ...
    await service.createPlans('c1', [
      { period: 1, planAmount: new Decimal(300), planDate: '2026-01-01' } as any,
      { period: 2, planAmount: new Decimal(400), planDate: '2026-02-01' } as any,
    ]);

    expect(prisma.paymentPlan.deleteMany).toHaveBeenCalledWith({ where: { contractId: 'c1' } });
    expect(prisma.paymentPlan.createMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.arrayContaining([expect.objectContaining({ contractId: 'c1', period: 1 })]),
      }),
    );
  });
  ...
  it('should createRecord and refresh plan status to PARTIAL', async () => {
    ...
    const tx = {
      paymentRecord: {
        ...
        aggregate: jest.fn().mockResolvedValue({ _sum: { amount: new Decimal(200) } }),
      },
      paymentPlan: {
        findUnique: jest.fn().mockResolvedValue({ id: 'p1', planAmount: new Decimal(500) }),
  ...
  it('should skip refreshPlanStatus update when plan is missing inside transaction', async () => {
    ...
    const tx = {
      paymentRecord: {
        create: jest.fn().mockResolvedValue({ id: 'r1' }),
        aggregate: jest.fn().mockResolvedValue({ _sum: { amount: new Decimal(100) } }),
      },
      paymentPlan: {
  ...
  it('should reject removePlan when plan not found', async () => {
    ...
    await expect(service.removePlan('missing')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should delete removePlan when plan has no records', async () => {
    ...
    expect(prisma.paymentPlan.delete).toHaveBeenCalledWith({ where: { id: 'p1' } });
  });

  it('should reject removeRecord when record not found', async () => {
    prisma.paymentRecord.findUnique.mockResolvedValueOnce(null);

    await expect(service.removeRecord('missing')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should removeRecord with plan and reconcile contract', async () => {
    ...
    const tx = {
      paymentRecord: {
        ...
        aggregate: jest.fn().mockResolvedValue({ _sum: { amount: new Decimal(500) } }),
      },
      paymentPlan: {
        findUnique: jest.fn().mockResolvedValue({ id: 'p1', planAmount: new Decimal(500) }),
    ...
    expect(tx.paymentRecord.delete).toHaveBeenCalledWith({ where: { id: 'r1' } });
    expect(tx.paymentPlan.update).toHaveBeenCalledWith({
      where: { id: 'p1' },
