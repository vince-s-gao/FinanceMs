...
describe('DepartmentsService', () => {
  ...
  it('should build an arbitrary-depth tree and keep sibling sort order', async () => {
    ...
    expect(result[0].children.map((item: any) => item.id)).toEqual(['child-a1', 'child-a2']);
  ...
  it('should retry create when generated department code conflicts', async () => {
    ...
    prisma.department.create.mockRejectedValueOnce(conflictError).mockResolvedValueOnce({ id: 'd-retry' });
  ...
  it('should evaluate isDepartmentCodeConflict helper branches', () => {
    expect((service as any).isDepartmentCodeConflict(new Error('x'))).toBe(false);
    ...
    expect((service as any).isDepartmentCodeConflict(wrongCodeError)).toBe(false);
    ...
    expect((service as any).isDepartmentCodeConflict(wrongTargetError)).toBe(false);

    const missingMetaError = Object.create((Prisma as any).PrismaClientKnownRequestError.prototype);
    missingMetaError.code = 'P2002';
    expect((service as any).isDepartmentCodeConflict(missingMetaError)).toBe(false);
  ...
  it('should reject update when setting itself as parent', async () => {
    ...
    await expect(service.update('d1', { parentId: 'd1' } as any)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('should reject update when parent is a child department', async () => {
    ...
    await expect(service.update('d1', { parentId: 'd3' } as any)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('should update department when parent relation is valid', async () => {
    ...
    prisma.department.findMany.mockResolvedValueOnce([{ id: 'd2' }]).mockResolvedValueOnce([]);
    prisma.department.update.mockResolvedValueOnce({ id: 'd1', parentId: 'd3', name: '更新后' });
  ...
  it('should toggle active status', async () => {
    ...
    prisma.department.update.mockResolvedValueOnce({ id: 'd1', isActive: false });
  ...
  it('should reject addMember when user does not exist', async () => {
    ...
    await expect(service.addMember('d1', 'u1')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should reject removeMember when user is not in department', async () => {
    jest.spyOn(service, 'findOne').mockResolvedValueOnce({ id: 'd1' } as any);
    prisma.user.findUnique.mockResolvedValueOnce({ id: 'u1', departmentId: 'd2' });

    await expect(service.removeMember('d1', 'u1')).rejects.toThrow(BadRequestException);
  });

  it('should remove member from department', async () => {
    jest.spyOn(service, 'findOne').mockResolvedValueOnce({ id: 'd1' } as any);
    prisma.user.findUnique.mockResolvedValueOnce({ id: 'u1', departmentId: 'd1' });
  ...
  it('should reject setManager when user does not exist', async () => {
    ...
    await expect(service.setManager('d1', 'u1')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should auto-assign manager to department and set managerId', async () => {
    ...
    prisma.user.findUnique.mockResolvedValueOnce({ id: 'u1', departmentId: 'd2' });
    prisma.user.update.mockResolvedValueOnce({ id: 'u1', departmentId: 'd1' });
    prisma.department.update.mockResolvedValueOnce({ id: 'd1', managerId: 'u1' });
  ...
  it('should throw when findOneWithMembers does not exist', async () => {
    ...
    await expect(service.findOneWithMembers('missing')).rejects.toThrow(NotFoundException);
  });
