...
describe("PaymentRequestsService", () => {
  ...
  beforeEach(() => {
    prisma = {
      ...
      project: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: "proj-1", isDeleted: false }),
      },
      contract: {
  ...
  it("should create payment request with generated requestNo", async () => {
    ...
    prisma.paymentRequest.findFirst.mockResolvedValueOnce({
      requestNo: "FK202603170009",
    });
  ...
  it("should throw when updating with non-existing project", async () => {
    ...
    await expect(
      service.update("pr-1", { projectId: "proj-missing" } as any),
  ...
  it("should throw when updating with non-existing bank account", async () => {
    ...
    await expect(
      service.update("pr-1", { bankAccountId: "bank-missing" } as any),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("should reject update when request is not draft", async () => {
    ...
    await expect(
      service.update("pr-1", { reason: "x" } as any),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("should submit draft request to pending", async () => {
    prisma.paymentRequest.update.mockResolvedValueOnce({
      id: "pr-1",
      status: "PENDING",
    });
    prisma.paymentRequest.findUnique.mockResolvedValueOnce({
  ...
  it("should reject confirmPayment when status is not approved", async () => {
    ...
    await expect(service.confirmPayment("pr-1")).rejects.toThrow(
      BadRequestException,
    );
  });
