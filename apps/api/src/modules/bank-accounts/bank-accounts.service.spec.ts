...
describe('BankAccountsService', () => {
  ...
  it('should reject duplicated account number during update', async () => {
    ...
    await expect(service.update('a1', { accountNo: 'dup' } as any)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('should clear other defaults when updating account as default', async () => {
    ...
    prisma.bankAccount.update.mockResolvedValueOnce({ id: 'a1', isDefault: true });

    await service.update('a1', { accountNo: '62220099', isDefault: true } as any);
  ...
  it('should toggle account enabled state', async () => {
    prisma.bankAccount.findUnique.mockResolvedValueOnce({ id: 'a1', isEnabled: true });
    prisma.bankAccount.update.mockResolvedValueOnce({ id: 'a1', isEnabled: false });
