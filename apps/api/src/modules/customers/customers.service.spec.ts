...
describe("CustomersService", () => {
  ...
  it("should evaluate isUniqueConflict helper branches", () => {
    expect((service as any).isUniqueConflict(new Error("x"), "code")).toBe(
      false,
    );

    const nonConflictError = Object.create(
      (Prisma as any).PrismaClientKnownRequestError.prototype,
    );
    nonConflictError.code = "P2025";
    nonConflictError.meta = { target: ["code"] };
    expect((service as any).isUniqueConflict(nonConflictError, "code")).toBe(
      false,
    );
    ...
    expect((service as any).isUniqueConflict(wrongTargetError, "code")).toBe(
      false,
    );
    ...
    expect((service as any).isUniqueConflict(missingMetaError, "code")).toBe(
      false,
    );

    const codeConflictError = Object.create(
      (Prisma as any).PrismaClientKnownRequestError.prototype,
    );
    codeConflictError.code = "P2002";
    codeConflictError.meta = { target: ["code"] };
    expect((service as any).isUniqueConflict(codeConflictError, "code")).toBe(
      true,
    );
  });
