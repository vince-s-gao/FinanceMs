...
describe('NotificationsService', () => {
  ...
  it('should return paginated list and unread count', async () => {
    const list = await service.findMine('u1', { page: 1, pageSize: 10, unreadOnly: 'true' } as any);
  ...
  it('should query all notifications when unreadOnly is false', async () => {
    await service.findMine('u1', { page: 2, pageSize: 5, unreadOnly: 'false' } as any);
  ...
  it('should return existing record when notification is already read', async () => {
    prisma.notification.findFirst.mockResolvedValueOnce({ id: 'n1', isRead: true });
  ...
  it('should throw when marking missing notification', async () => {
    ...
    await expect(service.markAsRead('u1', 'missing')).rejects.toThrow(NotFoundException);
  });
