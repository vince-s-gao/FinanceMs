...
describe("InvoicesService", () => {
  ...
  it("should throw in getInvoiceRisk when contract does not exist", async () => {
    prisma.contract.findFirst.mockResolvedValueOnce(null);
    await expect(service.getInvoiceRisk("missing")).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("should parse pdf text instead of binary metadata noise", async () => {
    ...
    jest
      .spyOn(service as any, "extractAttachmentText")
      .mockResolvedValue(
        [
          "电子发票（增值税专用发票）",
    ...
    const parsed = await (service as any).parseAttachmentFile(
      file,
      1,
      "contract-1",
    );
  ...
  it("should not parse long invoice number as amount for attachment pdf", async () => {
    const file = {
      originalname:
        "dzfp_26112000000065517391_国耀融汇融资租赁有限公司_20260106161428.pdf",
      buffer: Buffer.from("fake-pdf"),
      mimetype: "application/pdf",
      size: 4096,
    } as Express.Multer.File;
  ...
  it("should import parsed invoice files and save attachment", async () => {
    ...
    const createSpy = jest
      ...
      .mockResolvedValue({ id: "inv-1001" } as any);
