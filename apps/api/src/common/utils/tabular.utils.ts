...
export function parseTabularBuffer(fileBuffer: Buffer, fileName?: string): string[][] {
  const ext = getFileExtension(fileName);
  if (ext === '.xlsx' || ext === '.xls') {
...
export function resolveHeaderIndex(headerRow: string[], aliases: readonly string[]): number | undefined {
