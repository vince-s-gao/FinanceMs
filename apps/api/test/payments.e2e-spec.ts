...
describe('PaymentsController Flow (e2e-like)', () => {
  ...
  it('should route statistics and contract-level queries', async () => {
    serviceMock.getStatistics.mockResolvedValueOnce({ summary: { contractCount: 1 } });
  ...
  it('should create plan(s) and record', async () => {
    ...
    const planList = [{ period: 1, planAmount: 100 }, { period: 2, planAmount: 200 }] as any;
  ...
  it('should remove plan and record by id', async () => {
    ...
    serviceMock.removeRecord.mockResolvedValueOnce({ message: '回款记录删除成功' });
