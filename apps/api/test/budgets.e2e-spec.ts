...
describe("BudgetsController Flow (e2e-like)", () => {
  ...
  it("should parse year and pass to getDepartmentSummary", async () => {
    serviceMock.getDepartmentSummary.mockResolvedValueOnce({
      year: 2026,
      department: "市场部",
    });
    ...
    expect(serviceMock.getDepartmentSummary).toHaveBeenCalledWith(
      2026,
      "市场部",
    );
  ...
  it("should pass create/update/freeze/close/remove", async () => {
    ...
    serviceMock.toggleFreeze.mockResolvedValueOnce({
      id: "b2",
      status: "FROZEN",
    });
