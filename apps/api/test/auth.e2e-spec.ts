...
describe('AuthController Flow (e2e-like)', () => {
  ...
  it('should return auth url and set oauth state cookie', () => {
    ...
    expect(result.url).toContain('https://open.feishu.cn/open-apis/authen/v1/authorize');
    expect(feishuServiceMock.getAuthUrl).toHaveBeenCalledWith(expect.any(String));
  ...
  it('should exchange ticket via endpoint', async () => {
    ...
    const result = await controller.exchangeFeishuTicket({ ticket: 'ticket-2' }, res);

    expect(feishuServiceMock.exchangeLoginTicket).toHaveBeenCalledWith('ticket-2');
