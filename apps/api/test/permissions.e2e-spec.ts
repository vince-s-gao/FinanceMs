...
describe('PermissionsController Flow (e2e-like)', () => {
  ...
  it('should query all permission definitions', async () => {
    ...
    serviceMock.getAllFunctions.mockResolvedValueOnce([{ key: 'contract:create' }]);
  ...
  it('should route role-level permission operations', async () => {
    ...
    serviceMock.initializeDefaultPermissions.mockResolvedValueOnce({ message: '初始化完成' });
    ...
    const updatedFuncs = await controller.updateRoleFunctionPermissions('FINANCE', {
      functions: ['contract:create'],
    });
    ...
    expect(serviceMock.updateRoleMenuPermissions).toHaveBeenCalledWith('FINANCE', ['dashboard']);
