...
describe('ExpensesController Flow (e2e-like)', () => {
  ...
  it('should pass current user context for update/submit/remove', async () => {
    ...
    serviceMock.submit.mockResolvedValueOnce({ id: 'exp-3', status: 'PENDING' });
    ...
    expect(serviceMock.update).toHaveBeenCalledWith('exp-3', updateDto, 'u-1', 'EMPLOYEE');
  ...
  it('should call approve and pay with expected params', async () => {
    serviceMock.approve.mockResolvedValueOnce({ id: 'exp-4', status: 'APPROVED' });
