...
describe('PaymentRequestsController Flow (e2e-like)', () => {
  ...
  it('should approve with approver user id', async () => {
    serviceMock.approve.mockResolvedValueOnce({ id: 'pr-2', status: 'APPROVED' });
  ...
  it('should pass remark in confirmPayment', async () => {
    serviceMock.confirmPayment.mockResolvedValueOnce({ id: 'pr-3', status: 'PAID' });

    const result = await controller.confirmPayment('pr-3', { remark: '已线下打款' } as any);

    expect(serviceMock.confirmPayment).toHaveBeenCalledWith('pr-3', '已线下打款');
    expect(result.status).toBe('PAID');
  });

  it('should call submit/cancel/remove by id', async () => {
    serviceMock.submit.mockResolvedValueOnce({ id: 'pr-4', status: 'PENDING' });
    serviceMock.cancel.mockResolvedValueOnce({ id: 'pr-4', status: 'CANCELLED' });
